...
export default class AnyClass extends PureComponent {
  ...
  constructor(props) {
    ...
    this.columns = [
      ...
      {
        title: '来源科室',
        dataIndex: 'config_department_id_name',
        align: 'center',
      },
      {
        title: '状态',
  ...
  get_data = (params) => {
    ...
    request(urls.admin_config_doctor_list,{
      body:{...payload},
      success:list=>{
        this.setState({list});
      },
  ...
  jump = ({ type, record }) => {
    ...
    if(['编辑'].indexOf(type)!==-1){
      router.jump({
        dispatch,
        pathname: `/basics/doctor-edit/${id}`,
      });
    }else if(type==='查看'){
      router.jump({
        dispatch,
        pathname: `/basics/doctor-detail/${id}`,
      });
    }
  };
  ...
  handleSearch = (event) => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      if(dep){
        try {
          params.config_department_id = departments.find(v=>v.name === dep).id;
        }catch (e) {
  ...
  create = ()=>{
    ...
    router.jump({
      dispatch,
      pathname: '/basics/doctor-add',
    });
  };

  renderSimpleForm() {
    const { form: { getFieldDecorator } } = this.props;
    const {departments=[],dep='' } = this.state;
    let dataSource = [];
    if(!dep){
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={{ md: 8, lg: 24 }}>
          <Col md={8}>
            <FormItem {...formItemLayout} label="医生编码">
        ...
        <div style={{ overflow: 'hidden' }}>
          <span style={{ float: 'right', marginBottom: 24 }}>
            ...
            <a href={`${urls.admin_config_doctor_export}/?${dataHandler.obj2queryString({...queryParamForExport,...storageHandler.getCurrentUserInfo()})}`} style={{ marginLeft: 8 }} ><Button type="primary">导出</Button></a>
