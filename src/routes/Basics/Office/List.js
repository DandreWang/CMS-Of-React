...
export default class AnyClass extends PureComponent {
  ...
  get_data = (params) => {
    ...
    request(urls.admin_config_department_list,{
      body:{...payload},
      success:list=>{
        this.setState({list});
      },
  ...
  jump = ({ type, record }) => {
    ...
    if(['编辑'].indexOf(type)!==-1){
      router.jump({
        dispatch,
        pathname: `/basics/office-edit/${id}`,
      });
    }else if(type==='查看'){
      router.jump({
        dispatch,
        pathname: `/basics/office-detail/${id}`,
      });
    }
  };
  ...
  remove = ({record})=>{
    ...
    const {id} = record||{};
    request(urls.admin_config_department_delete,{
      body:{id},
      success:()=>{
        this.handleSearch();
      },
      ...
      <Form onSubmit={this.handleSearch} layout="inline">
        ...
        <div style={{ overflow: 'hidden' }}>
          <span style={{ float: 'right', marginBottom: 24 }}>
            ...
            <a href={`${urls.admin_config_department_export}/?${dataHandler.obj2queryString({...queryParamForExport,...storageHandler.getCurrentUserInfo()})}`} style={{ marginLeft: 8 }} ><Button type="primary">导出</Button></a>
