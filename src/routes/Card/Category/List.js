...
export default class AnyClass extends PureComponent {
  ...
  constructor(props) {
    ...
    this.columns = [
      ...
      // 分类编码（code）、分类名称（name）、面额（price，整数）、用途类型（use_type，数据来源：/common_status/detail，参数：分类-卡类用途）、状态（0，4）
      ...
      },
      {
        ...
        render:(text,record)=>{
          ...
          return <span>{use_type[text+'']}</span>
        },
      },
      {
        title: '状态',
  ...
  get_data = (params) => {
    ...
    request(urls.admin_consume_card_category_list,{
      body:{...payload},
      success:list=>{
        this.setState({list});
      },
  ...
  common_status_detail = () => {
    ...
    request(urls.common_status_detail, {
      ...
      method:'get',
      success: use_type => {
        this.setState({ use_type },()=>this.get_data());
      },
      fail: errmsg => {
        message.error(errmsg || '获取失败');
      },
  ...
  jump = ({ type, record }) => {
    ...
    if(['编辑'].indexOf(type)!==-1){
      ...
      router.jump({
        dispatch,
        pathname: `/card/category-detail/${id}`,
      });
    }
  };
  ...
  remove = ({record})=>{
    ...
    const {id} = record||{};
    request(urls.admin_consume_card_category_delete,{
      body:{id},
      success:()=>{
        this.handleSearch();
      },
      ...
      <Form onSubmit={this.handleSearch} layout="inline">
        ...
        <div style={{ overflow: 'hidden' }}>
          <span style={{ float: 'right', marginBottom: 24 }}>
            ...
            <a href={`${urls.admin_consume_card_category_export}/?${dataHandler.obj2queryString({...queryParamForExport,...storageHandler.getCurrentUserInfo()})}`} style={{ marginLeft: 8 }} ><Button type="primary">导出</Button></a>
