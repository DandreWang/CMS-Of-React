...
export default class AnyClass extends PureComponent {
        ...
        render: (text, record) => {
          ...
          return (
            <div className="editable-row-operations">
              <Fragment>
                <a onClick={() => this.jump({ type:'查看病历', record })}>查看病历</a>
  ...
  componentDidMount() {
    ...
    defaultParams = { type: 3, status: 0, current: 1, pageSize: 20 };
  ...
  get_data = (params) => {
    ...
    request(urls.get_filed_patient_case_list, {
      body: { ...payload },
      success: list => {
        this.setState({ list });
      },
  ...
  jump = ({ type, record }) => {
    const {case_id:id} = record;
    const {dispatch} = this.props;

    if(['查看病历'].indexOf(type)!==-1){
      router.jump({
        dispatch,
        pathname: `/patient-case/filed-detail/${id}`,
      });
    }else if(type==='查看报告'){
      router.jump({
        dispatch,
        newTab:true,
        pathname: `/patient/synthesis/${id}`,
      });
    }else if(type==='查看治疗记录'){
      router.jump({
        dispatch,
        pathname: `/patient/treat-record/${id}`,
      });
    }
  };
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={{ md: 8, lg: 15 }}>
          <Col md={8}>
            <FormItem {...formItemLayout} label="患者编号">
        ...
        <div style={{ overflow: 'hidden' }}>
          <span style={{ float: 'right', marginBottom: 24 }}>
            ...
            <a href={`${urls.export_filed_cases}/?${dataHandler.obj2queryString({...queryParamForExport,...storageHandler.getCurrentUserInfo()})}`} style={{ marginLeft: 8 }} ><Button type="primary">导出</Button></a>
