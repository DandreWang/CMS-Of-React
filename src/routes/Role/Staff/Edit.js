...
export default class AnyClass extends React.Component {
    ...
    request(urls.admin_role_list, {
      ...
      success: data => {
        if(data&&data.list&&data.list.length){
          this.setState({ role_list:data.list});
        }
      },
      fail: errmsg => {
        message.error(errmsg || '获取失败');
  ...
  admin_account_get = () => {
    ...
    request(urls.admin_account_get, {
      body: { id },
      success: detail => {
        this.setState({ detail,role_level:detail.role_level });
      },
      fail: errmsg => {
        message.error(errmsg || '获取失败');
      },
  ...
  render() {
    ...
    const {role_list,department_list,role_level,edu_list,marriage_list,detail={},loading} = this.state;
    const { styles: { formItemLayout, tailFormItemLayout } } = tableHandler;

    const {role_id,department_id,account_name,account_mobile,account_pwd,true_name,sex,id_card,edu_level,marriage_type} = detail||{};
