...
export default class TreatPlan extends PureComponent {
  ...
  get_medicine_list = () => {
    request(urls.get_medicine_list, {
      body: { case_id }, //todo 不需要case_id，待测
  ...
  // 取消弹框或者继续
  handleOk = () => {
    const { diseaseLevelValue, diseaseLevelIndex, diseaseLevelType, impression_list, secondary_impression_list } = this.state;
  ...
  submit = () => {
    const { dispatch } = this.props;
    const { impression_desc, treatments } = this.state;
    ...
    const body = {
      case_id, impression_desc, treatments: JSON.stringify(t), medicines: JSON.stringify(medicines),
      level1_diagnosis_id,
      level2_diagnosis_id,
      symptom_diagnosis, pre_diagnosis, medical_analysis,
    };
    ...
    request(urls.save_diagnosis, {
      body,
      success: () => {
        router.jump({
          operator: 'replace',
          dispatch,
          pathname: `/patient/patient-diagnosis-success/${case_id}`,
        });
      },
      fail: errmsg => {
  ...
  addTreatList = () => {
    ...
    const diff = lodash.differenceBy(medicine_options, m, 'medicine_id');
    m.push({
      medicine_id: diff[0].medicine_id,
      medicine_name: diff[0].medicine_name,
      medicine_use_type: diff[0].medicine_use_type,
    });
    this.setState({
      medicines: m,
    });
  ...
  onChangeOfMedicines = (value, index, type) => {
    ...
    if (type === 99) {
      if (m.find(value => value.medicine_id - 0 === v)) {
        return message.error('请勿重复选择');
      } else {
  ...
  onChangeOfCureConfig = (index) => {
    const { medicines = [], treatments = [] } = this.state;
    const medicinesCopy = dataHandler.deepClone(medicines);
    const treatmentsCopy = dataHandler.deepClone(treatments);
    // todo
    this.setState({
          ...
          <Collapse defaultActiveKey={['心身测评结果', '诊断结论', '治疗方案']}>
            ...
            <Panel header="诊断结论" key="诊断结论">
              ...
              <div className={styles.rowDia} style={{ marginTop: 20, alignItems: 'flex-start' }}>
                ...
                <TextArea autosize={{ minRows: 4, maxRows: 12 }} placeholder='选填' onChange={(event) => {
                  this.onChangeOfArea(event, 1);
                }}/>
              ...
              <div className={styles.rowDia} style={{ marginTop: 20, alignItems: 'flex-start' }}>
                ...
                <label className={`${styles.title}`}
                       style={{ width: 137, whiteSpace: 'wrap' }}>整体医学分析（包括相关心身因素及应激）：</label>

                <TextArea autosize={{ minRows: 4, maxRows: 12 }} placeholder='选填' onChange={(event) => {
                  this.onChangeOfArea(event, 3);
                }}/>
                    ...
                    return (
                      <div className={styles.rowFlex} key={`${item.medicine_id}${index}`}>
                        <div className={styles.colFlex}>
                          <label style={index !== 0 ? { visibility: 'hidden' } : {}}>用药选择：</label>
                          <Select key={`Select${item.medicine_id}${index}`}
                                  onChange={(value) => this.onChangeOfMedicines(value, index, 99)}
                                  value={item.medicine_id} style={{ width: 200 }}>
                            {
                              medicine_options.map((value, i) => {
                                return (
                                  <Option key={`Option${value.medicine_id}${index}${i}`}
                                          value={value.medicine_id}>{value.medicine_name}</Option>
                                );
                              })
                            }
                          </Select>
                        ...
                        <div className={styles.colFlex1}>
                          <label>用量：</label>
                          <InputNumber min={1} style={{ width: 60 }} value={item.use_amount - 0 || ''}
                                       onChange={(value) => this.onChangeOfMedicines(value, index, 0)}/>
                          <label htmlFor=""> {unit}</label>
                        </div>
                        <div className={styles.colFlex1}>
                          <label>天数：</label>
                          <InputNumber min={1} style={{ width: 60 }} value={item.use_days - 0 || ''}
                                       onChange={(value) => this.onChangeOfMedicines(value, index, 1)}/>
                        </div>
                        <div className={styles.colFlex1}>
                          <label>用法：</label>
                          <InputNumber min={1} style={{ width: 60 }} value={item.use_rate - 0 || ''}
                                       onChange={(value) => this.onChangeOfMedicines(value, index, 2)}/>
                          ...
                          return (
                            <Col span={7} className={styles.linkAge} key={value.item_id}>
                              <Checkbox checked={!!value.checked}
                                        onChange={(event) => this.testCheck(event, value.item_id)}>{value.item_name}</Checkbox>
                              {
                                value.checked ? (
                                  <Fragment>
                                    <InputNumber min={1}
                                                 onChange={(number) => this.handleNumberChange(number, value.item_id)}
                                                 defaultValue={value.item_count || ''}/>
