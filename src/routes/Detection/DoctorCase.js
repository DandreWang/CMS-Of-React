...
export default class AnyClass extends PureComponent {
  ...
  constructor(props) {
    ...
    this.columns = [
      ...
      },
      {
        ...
        // width: 180,
        render: (text, record) => {
          const { case_status, is_finish_test, is_finish_diagnosis } = record;
          return (
            <div className="editable-row-operations">
              <Fragment>
                <a onClick={() => this.jump({ type: '查看报告', record })}>查看报告</a>
                <Divider type="vertical"/>
                <a onClick={() => this.jump({ type: '开始诊断', record })}>开始诊断</a>
                {
                  getAuthority()==='doctor'?(
                    <Fragment>
                      <Divider type="vertical" />
  ...
  jump = ({ type, record }) => {
    ...
    if (type === '查看报告') {
      ...
      router.jump({
        dispatch,
        pathname: `/patient/treat-plan/${case_id}`,
      });
    }
  };
  edit = ({ record }) => {
