...
const filterStatusMap = {'0':'全部','1':'诊断完成','2':'治疗中','3':'治疗完成','9':'治疗中断'};
...
export default class AnyClass extends PureComponent {
          ...
          return (
            <div className="editable-row-operations">
              <Fragment>
                ...
                <a onClick={() => this.jump({ type:'复诊', record })}>复诊</a>
                {
                  getAuthority()==='doctor'?(
                    <Fragment>
                      <Divider type="vertical" />
                      ...
                      <Divider type="vertical" />
                      <Popconfirm title="归档后不可再修改病历基本信息，确认归档？" onConfirm={() => this.filed({ record })}>
                        <a>归档</a>
                      </Popconfirm>
                    </Fragment>
                  ):null
  ...
  componentDidMount() {
    ...
    defaultParams = {type:4,status:0,current:1,pageSize:20};
  ...
  jump = ({ type, record }) => {
    ...
    if(type==='查看报告'){
      ...
      router.jump({
        dispatch,
        pathname: `/patient/treat-plan-again/${case_id}`,
      });
    }
  };
  edit = ({ record }) => {
    ...
    router.jump({
      dispatch,
      pathname: `/patient/patient-case-edit/${case_id}`,
    });
  };
  filed = ({ record }) => {
            ...
            <FormItem {...formItemLayout} label="病历状态">
              {getFieldDecorator('status', {
                ...
                <Select placeholder="请选择" className={styles.selectForm}>
                  {
                    Object.keys(filterStatusMap).map((v,i) => {
                      return (
                        <Option key={v} value={v}>{filterStatusMap[v]}</Option>
                      );
                    })
                  }
                </Select>,
