...
export default class AnyClass extends PureComponent {
          ...
          if(3===case_status-0){
            return (
              <div className="editable-row-operations">
                <Fragment>
                  <a onClick={() => this.jump({ type:'开始治疗', record })}>开始治疗</a>
                  <Divider type="vertical" />
                  <a onClick={() => this.jump({ type:'查看报告', record })}>查看报告</a>
                </Fragment>
              </div>
            );
          }else if([4].indexOf(case_status-0)!==-1){
            return (
              <div className="editable-row-operations">
                <Fragment>
                  <a onClick={() => this.jump({ type:'继续治疗', record })}>继续治疗</a>
                  <Divider type="vertical" />
                  <a onClick={() => this.jump({ type:'查看报告', record })}>查看报告</a>
                  <Divider type="vertical" />
  ...
  componentDidMount() {
    ...
    defaultParams = {type:5,status:0,current:1,pageSize:20};
  ...
  jump = ({ type, record }) => {
    ...
    if(type==='开始治疗'||type==='继续治疗'){
      router.jump({
        dispatch,
        pathname: `/patient/treat/${case_id}`,
      });
