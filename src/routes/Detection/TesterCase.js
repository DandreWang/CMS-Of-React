...
const filterStatusMap = ['全部','待测评','测评中'];
...
export default class AnyClass extends PureComponent {
  ...
  constructor(props) {
    ...
    this.columns = [
      ...
      },
      {
        ...
        // width: 180,
        render: (text, record) => {
          const {case_status,is_finish_test} = record;
          const status = case_status-0;
          const descArr = ['开始测评','继续测评'];
          return (
            <div className="editable-row-operations">
              <Fragment>
                <a onClick={() => this.jump({ record })}>{descArr[status]}</a>
                {
                  getAuthority()==='doctor'?(
                    <Fragment>
  ...
  componentDidMount() {
    ...
    defaultParams = {type:1,status:0,current:1,pageSize:20};
