...
export default class AnyClass extends PureComponent {
  ...
  get_data = (params) => {
    ...
    request(urls.admin_evaluate_subject_category_list,{
      body:{...payload},
      success:list=>{
        this.setState({list});
      },
  ...
  jump = ({ type, record }) => {
    ...
    if(['编辑'].indexOf(type)!==-1){
      router.jump({
        dispatch,
        pathname: `/scale/category-edit/${id}`,
      });
    }else if(type==='查看'){
      router.jump({
        dispatch,
        pathname: `/scale/category-detail/${id}`,
      });
    }
  };
  ...
  remove = ({record})=>{
    ...
    const {id} = record||{};
    request(urls.admin_evaluate_subject_category_delete,{
      body:{id},
      success:()=>{
        this.handleSearch();
      },
      ...
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={{ md: 8, lg: 24 }}>
          ...
          <Col md={8}>
            ...
            <a style={{ marginLeft: 8 }} href={`${urls.admin_evaluate_subject_category_export}/?${dataHandler.obj2queryString({...queryParamForExport,...storageHandler.getCurrentUserInfo()})}`} >
              <Button type="primary">
                导出
              </Button>
            </a>
