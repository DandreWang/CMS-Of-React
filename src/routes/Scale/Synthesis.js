...
import styles from './Synthesis.less';
...
const ResultDescrip = ({ data: { group_name, calculate_level_name, calculate_level_desc }, i, j }) => (
  <div className={styles.resultDescripWrapper} key={group_name}>
    ...
    {calculate_level_desc && calculate_level_desc !== '无' ? (
      <p className={styles.describe}>{calculate_level_desc}</p>) : null}
  </div>
);

// 设置标题样式
const MyTitle = ({ title, className }) => <h3 className={className}>{title}</h3>;
let case_id;
@connect(() => ({}))
@Form.create()
export default class Synthesis extends PureComponent {
    ...
    return (
      <Fragment>
...

        {
          hasSecondLevel?(
            <Fragment>
              <MyTitle title="整体医学评估结果" className={styles.font_h3}/>
                ...
                <div className={styles.pieWrapper}>
                  {
                    pieDesc.map((item, idx) => (
                      <div className={styles.pieItem} key={item.group_name}>
                        <div
                          className={styles.pieTit}>{`${idx + 1}、${item.group_name}：${item.calculate_level_name}`}</div>
  ...
  tab2 = () => {
    ...
    try {
      ...
      return evaluate_test_report && evaluate_test_report.length ? (
        evaluate_test_report.map((report, index) => {
          ...
          return (
            <div className={styles.blockLevel} key={`report${index}`}>
  ...
  tab3 = () => {
    ...
    return device_report && device_report.length ? device_report.map((value, i) => {
      const { test_img, device_category_name, conclusion, device_category_id } = value || {};
      return (
        <Fragment key={`device_category_id${device_category_id}`}>
          <Card type="inner" title={`${device_category_name}报告`} className={styles.blockLevel}
                style={{ marginBottom: 0, marginTop: i !== 0 ? 15 : 0 }}>
            <div className={styles.testContent}>
              <img src={test_img} alt="" className={styles.img}/>
            </div>
            <div className={styles.conclusion}>结论: {conclusion || '无'}</div>
          </Card>
        </Fragment>
      );
    }) : (<div style={{ textAlign: 'center' }}>暂无相关信息</div>);
  ...
  tab4 = () => {
    const { patient_case_test_report = {} } = this.state;
    const { chief_report, diagnosis_report } = patient_case_test_report || {};
    const {
      create_time, diagnosis_selections, assist_diagnosis_selections, symptom_diagnosis, pre_diagnosis, medical_analysis,
          ...
          <Card type="inner" title="诊断结论" className={styles.cardItem}>
            {
              diagnosis_selections ? (
                <DescriptionList className={styles.cardList} size="small" col="1">
                  <Description term="诊断选项"
                               termclass={styles.termStyle}>{diagnosis_selections}</Description>
                </DescriptionList>
              ) : null
            }
            {
              assist_diagnosis_selections ? (
                <DescriptionList className={styles.cardList} size="small" col="1">
                  <Description term="辅助诊断选项"
                               termclass={styles.termStyle}>{assist_diagnosis_selections}</Description>
                </DescriptionList>
              ) : null
            }
            {
              symptom_diagnosis ? (
                <DescriptionList className={styles.cardList} size="small" col="1">
                  <Description term="症状学诊断"
                               termclass={styles.termStyle}>{symptom_diagnosis}</Description>
                </DescriptionList>
              ) : null
            }
            {
              pre_diagnosis ? (
                <DescriptionList className={styles.cardList} size="small" col="1">
                  <Description term="初步诊断"
                               termclass={styles.termStyle}>{pre_diagnosis}</Description>
                </DescriptionList>
              ) : null
            }
            {
              medical_analysis ? (
                <DescriptionList className={styles.cardList} size="small" col="1">
                  <Description term="整体医学分析"
                               termclass={styles.termStyle}>{medical_analysis}</Description>
                </DescriptionList>
              ) : null
            }
  ...
  export = () => {
    ...
    // /#/patient/synthesis/23

    const { dispatch } = this.props;
    router.jump({
      dispatch,
      newTab: true,
      pathname: `/patient/synthesis-for-export/${case_id}`,
    });
