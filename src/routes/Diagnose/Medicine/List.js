...
export default class AnyClass extends PureComponent {
  ...
  get_data = (params) => {
    ...
    request(urls.admin_medicine_list,{
      body:{...payload},
      success:list=>{
        this.setState({list});
      },
  ...
  jump = ({ type, record }) => {
    ...
    if(['编辑'].indexOf(type)!==-1){
      ...
      router.jump({
        dispatch,
        pathname: `/diagnose/medicine-detail/${id}`,
      });
    }
  };
  ...
  create = ()=>{
    ...
    router.jump({
      dispatch,
      pathname: '/diagnose/medicine-add',
    });
  };
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={{ md: 8, lg: 24 }}>
          <Col md={8}>
            <FormItem {...formItemLayout} label="药物编码">
          ...
          <Col md={8}>
            <FormItem {...formItemLayout} label="药物名称">
          ...
          <Col md={8}>
            <span className={styles.submitButtons}>
              ...
              <a href={`${urls.admin_medicine_export}/?${dataHandler.obj2queryString({...queryParamForExport,...storageHandler.getCurrentUserInfo()})}`} style={{ marginLeft: 8 }} ><Button type="primary">导出</Button></a>
