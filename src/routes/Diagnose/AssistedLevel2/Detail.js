...
export default class AnyClass extends React.Component {
  ...
  get_data = () => {
    ...
    request(urls.admin_config_assist_diagnosis_level2_get, {
      body: { id },
      success: items => {
        this.setState({ ...items });
  ...
  render() {
    ...
    const {loading,
      code,name,price,use_type,parent_id_name='',use_type_obj={}, status,
    } = this.state;
      ...
      <PageHeaderLayout wrapperClassName={styles.wrapperClassName}>
        ...
        >
          ...
          >
            ...
            <Description className={styles.infoItem} term="所属一级诊断参数">{parent_id_name||'无'}</Description>
