...
export default class AnyClass extends PureComponent {
  ...
  onClickOfMenu = (data,record)=>{
    ...
    if(key-0===1){
      dispatch(routerRedux.push({
        pathname:'/diagnose/assisted/level2-list',
        payload:{
          level1_id:id,
        },
      }));
  ...
  get_data = (params) => {
    ...
    request(urls.admin_config_assist_diagnosis_level1_list,{
      body:{...payload},
      success:list=>{
        this.setState({list});
      },
  ...
  jump = ({ type, record }) => {
    ...
    if(['编辑'].indexOf(type)!==-1){
      router.jump({
        dispatch,
        pathname: `/diagnose/assisted/level1-edit/${id}`,
      });
    }else if(type==='查看'){
      router.jump({
        dispatch,
        pathname: `/diagnose/assisted/level1-detail/${id}`,
      });
    }
  };
  ...
  remove = ({record})=>{
    ...
    const {id} = record||{};
    request(urls.admin_config_assist_diagnosis_level1_delete,{
      body:{id},
      success:()=>{
        this.handleSearch();
      },
  ...
  create = ()=>{
    ...
    router.jump({
      dispatch,
      pathname: '/diagnose/assisted/level1-add',
    });
  };
      ...
      <Form onSubmit={this.handleSearch} layout="inline">
        ...
        <div style={{ overflow: 'hidden' }}>
          <span style={{ float: 'right', marginBottom: 24 }}>
            ...
            <a href={`${urls.admin_config_assist_diagnosis_level1_export}/?${dataHandler.obj2queryString({...queryParamForExport,...storageHandler.getCurrentUserInfo()})}`} style={{ marginLeft: 8 }} ><Button type="primary">导出</Button></a>
