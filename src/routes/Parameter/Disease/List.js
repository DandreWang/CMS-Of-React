...
export default class AnyClass extends PureComponent {
  ...
  constructor(props) {
    super(props);
    this.columns = [
      // code、name、department_id_name、bind_treatment_flow_template_id_name、create_time_str、create_worker_account_id_account_name
      {
        title: '疾病编码',
        dataIndex: 'code',
        align: 'center',
      ...
      {
        title: '绑定测评流程模板',
        dataIndex: 'bind_treatment_flow_template_id_name',
        align: 'center',
      },
      {
        title: '创建时间',
  ...
  get_data = (params) => {
    ...
    request(urls.admin_disease_list,{
      body:{...payload},
      success:list=>{
        this.setState({list});
      },
  ...
  jump = ({ type, record }) => {
    ...
    if(['编辑'].indexOf(type)!==-1){
      ...
      router.jump({
        dispatch,
        pathname: `/parameter/disease-detail/${id}`,
      });
    }
  };
  ...
  create = ()=>{
    ...
    router.jump({
      dispatch,
      pathname: '/parameter/disease-add',
    });
  };
  ...
  remove = ({record})=>{
    ...
    const {id} = record||{};
    request(urls.admin_disease_delete,{
      body:{id},
      success:()=>{
        this.handleSearch();
      },
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          <Col md={8}>
            <FormItem {...formItemLayout} label="疾病编码">
        ...
        <div style={{ overflow: 'hidden' }}>
          <span style={{ float: 'right', marginBottom: 24 }}>
            ...
            <a href={`${urls.admin_disease_export}/?${dataHandler.obj2queryString({...queryParamForExport,...storageHandler.getCurrentUserInfo()})}`} style={{ marginLeft: 8 }} ><Button type="primary">导出</Button></a>
