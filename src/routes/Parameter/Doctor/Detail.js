...
export default class AnyClass extends React.Component {
  ...
  componentDidMount() {
    ...
    this.get_form_doctor_detail();

  }


  get_form_doctor_detail = () => {
  ...
  edit = ()=>{
    ...
    router.jump({
      dispatch,
      pathname: `/parameter/doctor-edit/${id}`,
    });
  };
  ...
  render() {
    ...
    const {
      ...
      manager_name,manager_mobile,status,config_department_id_name,
    } = items||{};
      ...
      <PageHeaderLayout wrapperClassName={styles.wrapperClassName}>
        ...
        >
          <DescriptionList
            ...
            col={3}
          >
            <Description className={styles.infoItem} term="医生编码">{code||'无'}</Description>
            <Description className={styles.infoItem} term="医生名称">{name||'无'}</Description>
            {/*<Description className={styles.infoItem} term="负责人姓名">{manager_name||'无'}</Description>*/}
            <Description className={styles.infoItem} term="来源科室">{config_department_id_name||'无'}</Description>
