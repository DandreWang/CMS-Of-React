...
export default class MsgRecordManage extends Component {
  state = {
    ...
    detail:{
      device_categories:[],
      subject_ids:[],
      is_statement_step_open:0,
      is_device_step_open:0,
      is_questionnaire_step_open:0,
      questionnaire_id:0,
      is_evaluate_step_open:0,
    },
    questionnaire:[], // 首访
  };
    ...
    request(urls.questionnaire_form_list, {
      ...
      success: questionnaire => {
        if(questionnaire&&questionnaire.length){
          this.update_questionnaire_id(questionnaire[0].id);
        }
  ...
  update_questionnaire_id = (questionnaire_id)=>{
    const {detail={}} = this.state;
    const d = dataHandler.deepClone(detail);
    d.questionnaire_id = questionnaire_id;
    this.setState({detail:d});
  }
  ...
  admin_test_flow_template_save = ()=>{
    ...
    request(urls.admin_test_flow_template_save, {
      body: {code,name,is_statement_step_open,is_device_step_open,is_questionnaire_step_open,is_evaluate_step_open,questionnaire_id,
      device_categories:JSON.stringify(device_categories),subject_ids:JSON.stringify(subject_ids),
      },
      success: data => {
              ...
              <div className={styles.mainCol}>
                ...
                </div>
                {
                  ...
                  (
                    <div>
                      {
                        devicesAll&&devicesAll.length?(
                          devicesAll.map((value,i)=>{
                          return (
              ...
              <div className={styles.mainCol}>
                ...
                </div>
                {
                  ...
                  (
                    <div>
                      {
                        subject_ids&&subject_ids.map((item,index) =>(
                              ...
                              <Select value={item} onChange={(value)=>this.onChangeOfEvaluate(value,index)}>
                                {
                                  evaluateAll&&evaluateAll.length?(
                                    ...
                                    return (
                                      <Option key={`evaluate-${value.id}-${j}`} value={value.id}>{value.name}</Option>
                                    )
                                  })
                                  ):null
                                }
                      ...
                      {
                        evaluateAll.length>subject_ids.length?(
